...
import time
# These are global variables whose values will be maintained between calls

example_global_variable = 0 
time_stamp = 0

def update_plot(data, axs, status_bar):
    global example_global_variable
    global time_stamp
    if example_global_variable == 0:
        time_stamp = time.time()
    example_global_variable += 1
    if example_global_variable % 10 == 0:
        status_bar.setText('%s\n%f' % (example_global_variable, (time.time()-time_stamp)/10))
        time_stamp = time.time()
"""
...
def update_plot(data, axs, status_bar):
    ...
    status_message = 'Number of events when the pulse trigger did not arrive: %d\n' % pulse_trigger_not_arrive_count
    status_message += 'Failure rate: %.1f %%' % (100*pulse_trigger_not_arrive_count / (pulse_trigger_not_arrive_count + sum(arrival_time)))
