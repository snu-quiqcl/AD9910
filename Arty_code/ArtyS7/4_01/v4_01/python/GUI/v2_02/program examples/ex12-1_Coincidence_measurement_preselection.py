...
Doppler_cooling_time = 50-3          # Doppler cooling time: 200 * 10ns
...
max_time_diff_to_record = 130      # If the time difference between PMT and pulse trigger is less than zero or >= max_time_diff_to_record, it won't be counted.

trigger_precut = 5               # if trigger comes before precut(tick), ignore that.

PMT_back_scattering_cut = 47            # if PMT fires before cut(tick), ignore that. 
...
s.load_immediate(PMT_cut_R, PMT_back_scattering_cut)
...
#\
#s.branch_if_less_than('calculate_time_PMT1', pulse_trigger_timing_R, PMT1_timing_R)
...
s.check_PMT1_cut = \
\
s.branch_if_less_than('store_time_PMT1', PMT_cut_R, time_diff1_R)
s.jump('check_PMT2_triggered')
...
s.check_PMT2_cut = \
\
s.branch_if_less_than('store_time_PMT2', PMT_cut_R, time_diff2_R)
s.jump('decide_repeat')
...
#s.write_to_fifo(PMT1_timing_R, PMT2_timing_R, pulse_trigger_timing_R, 111, 'Store time difference')
...
def save_data_as_file(data, header, run_count):
    ...
    with open(filename2, 'w') as f:
        ...
        for x in range(len(PMT1_arrival_time)):
            f.write('%f, %d, %d\n' %(x, PMT1_arrival_time[x],PMT2_arrival_time[x]))
