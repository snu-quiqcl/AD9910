...
class power_table():
    def __init__(self, filename, min_power=-60, max_power=-3.5):
        ...
        with open(filename) as csvfile:
            ...
            for row in freq_power_data:
                ...
                if float(row[3]) >= self.min_power: # Data under -60 dBm is not reliable due to noise
    ...
    def set_power_limit(self, min_power, max_power):
        self.min_power = min_power
        self.max_power = max_power
    ...
    def voltage_for_freq_power(self, freq_in_MHz, power_in_dBm):
        if (freq_in_MHz < 10) or (freq_in_MHz > 100):
            ...
            raise ValueError()
        if (power_in_dBm < self.min_power) or (power_in_dBm > self.max_power):
            print('Error in voltage_for_freq_power: power should be between -60 dBm and -3.5 dBm')
            raise ValueError()
...
if __name__ == '__main__':
    pt = power_table('calibration_DDS1_DAC1_HSTL_FB_connected_DDS_full_current_ZX73-2500-S+_180215.csv', -60, -3.5)
    #pt = power_table('calibration_DDS2_DAC0_DDS_full_current_ZX73-2500-S+_180215', -60, 1.0)
