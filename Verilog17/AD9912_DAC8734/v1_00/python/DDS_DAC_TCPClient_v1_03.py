...
class DDS_DAC(QtWidgets.QDialog, Ui_QDialog):
    ...
    def DDS1_read_power_limits(self):
        dBm = float(self.query('Q:DDS1:MAX_POWER'))
        mW = 10**(dBm/10)
        Vamp = math.sqrt(mW/10)
        self.DDS1_dBm_spinbox.setMaximum(dBm)
        self.DDS1_mW_spinbox.setMaximum(mW)
        self.DDS1_Vpp_spinbox.setMaximum(2*Vamp)

        dBm = float(self.query('Q:DDS1:MIN_POWER'))
        mW = 10**(dBm/10)
        Vamp = math.sqrt(mW/10)
        self.DDS1_dBm_spinbox.setMinimum(dBm)
        self.DDS1_mW_spinbox.setMinimum(mW)
        self.DDS1_Vpp_spinbox.setMinimum(2*Vamp)


    def DDS2_read_power_limits(self):
        dBm = float(self.query('Q:DDS2:MAX_POWER'))
        mW = 10**(dBm/10)
        Vamp = math.sqrt(mW/10)
        self.DDS2_dBm_spinbox.setMaximum(dBm)
        self.DDS2_mW_spinbox.setMaximum(mW)
        self.DDS2_Vpp_spinbox.setMaximum(2*Vamp)

        dBm = float(self.query('Q:DDS2:MIN_POWER'))
        mW = 10**(dBm/10)
        Vamp = math.sqrt(mW/10)
        self.DDS2_dBm_spinbox.setMinimum(dBm)
        self.DDS2_mW_spinbox.setMinimum(mW)
        self.DDS2_Vpp_spinbox.setMinimum(2*Vamp)
